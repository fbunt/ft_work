...
dropped_dates = []
idx = []
it = tqdm.tqdm(enumerate(zip(tb_ds, era_ds)), ncols=80, total=len(era_ds))
for i, (tbi, erai) in it:
    tbi = tbi.numpy()
    nan = np.isnan(tbi)
    if not nan.all():
...
dropped_dates = []
idx = []
it = tqdm.tqdm(enumerate(zip(tds15, era_ds)), ncols=80, total=len(era_ds))
for i, (tbi, erai) in it:
    tbi = tbi.numpy()
    nan = np.isnan(tbi)
    if not nan.all():
